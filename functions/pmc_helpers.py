...
def doi_sourced(doi, timeout = (10,60)):
    output = {'status': None,
              'error': None,
              'output': None}
...
def doi_to_pmcid(doi, timeout = (10,60)):
    if doi is None or doi == '':
        return None
    r = doi_sourced(doi, timeout = timeout)
...
def find_pmc_fulltext_pdf_ftplink(pmcid,timeout=(10,60)):
    if pmcid is None:
        return None
 ...
 # create a header to interact with the crossref API and server appropriately
    ...
    if len(pdf_links) > 0:
        output = pdf_links[0]['href']
    elif len(tgz_links) > 0:
        output = tgz_links[0]['href']
    else:
        output = construct_pmc_fulltext_pdf_link(pmcid) 
